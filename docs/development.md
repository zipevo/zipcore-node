...
To develop bitcore-node-zip:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-zip.git
...
We now will setup symlinks in `bitcore-node-zip` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-zip
ln -s ~/bitcoind-rpc-zip
```
...
cd devnode
mkdir node_modules
touch bitcore-node-zip.json
touch package.json
```

Edit `bitcore-node-zip.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-zip/bin/bitcore-node-zip start
```