...
Dash.prototype._initZmqSubSocket = function(node, zmqUrl) {
  ...
  return new Promise((resolve) => {
    ...
    node.zmqSubSocket.on('connect:delay', function(info) {
      log.warn('ZMQ connection delay:', info.address);
    });

    node.zmqSubSocket.on('disconnect', function(info) {
      log.warn('ZMQ disconnect:', info.address);
    });

    const socketOptions = { reconnectInterval: 1500 };
